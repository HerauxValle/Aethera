...
Singleton {
    ...
    function _bindSpec(action, bindStr) {
        ...
        const mods  = parts.slice(0, parts.length - 1).join(" ")
    ...
    function _updateBind(action, newBindStr) {
        ...
        if (old && old !== newBindStr) _runHyprctl(["hyprctl", "keyword", "unbind", _unbindSpec(old)])
        if (newBindStr) {
            _runHyprctl(["hyprctl", "keyword", "bind", _bindSpec(action, newBindStr)])
    ...
    function applyAllBinds() {
        // Scrape hyprctl binds to remove ANY stale bind pointing at our IPC path,
        // then register the current set fresh.
    ...
    Process {
        ...
        stdout: StdioCollector { id: _cleanupOut }
        onExited: {
            try {
                const binds = JSON.parse(_cleanupOut.text.trim())
                for (const b of binds) {
                    if (!b.arg || !b.arg.includes("qs ipc")) continue
                    const mm = b.modmask; const parts = []
                    if (mm & 64) parts.push("SUPER")
                    if (mm & 1)  parts.push("SHIFT")
                    if (mm & 4)  parts.push("CTRL")
                    if (mm & 8)  parts.push("ALT")
                    BarConfig._runHyprctl(["hyprctl", "keyword", "unbind",
                        parts.join(" ") + ", " + b.key])
                }
            } catch(e) {}
            // Register all current binds after cleanup
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("drawer",        BarConfig.bindDrawer)])
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("launcher",      BarConfig.bindLauncher)])
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("controlcenter", BarConfig.bindControlCenter)])
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("power",         BarConfig.bindPower)])
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("settings",      BarConfig.bindSettings)])
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("notifications", BarConfig.bindNotifications)])
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("wifi",          BarConfig.bindWifi)])
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("bluetooth",     BarConfig.bindBluetooth)])
            BarConfig._runHyprctl(["hyprctl", "keyword", "bind", BarConfig._bindSpec("workspacemenu", BarConfig.bindWorkspaceMenu)])
            // Sync _activeBinds to match what we just registered
            BarConfig._activeBinds = {
                "drawer":        BarConfig.bindDrawer,
                "launcher":      BarConfig.bindLauncher,
                "controlcenter": BarConfig.bindControlCenter,
                "power":         BarConfig.bindPower,
                "settings":      BarConfig.bindSettings,
                "notifications": BarConfig.bindNotifications,
                "wifi":          BarConfig.bindWifi,
                "bluetooth":     BarConfig.bindBluetooth,
                "workspacemenu": BarConfig.bindWorkspaceMenu
            }
        }
    }
